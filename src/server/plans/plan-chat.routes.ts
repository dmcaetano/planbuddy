  ...
  asyncHandler(async (req, res) => {
    ...
    if (action.action === "react" && action.reaction) {
      ...
      if (currentSpec.generationCount >= 2) {
        ...
        const generated = await runGeneration(req.user!.id, currentSpec, currentSpec.generationCount);
        const count = await incrementGenerationCount(currentSpec.id);
        revision = revisionResponse({ ...currentSpec, generationCount: count }, generated, req.user!.id);
        reply = generated.winner ? "Here is another grounded option. Your previous version is still available." : "I couldn't find another option that cleared every constraint.";
      }
      ...
      if (generated.winner) {
        revision = revisionResponse({ ...child, generationCount: count }, generated, req.user!.id);
        ...
        reply = `${scopeCopy} The original remains one tap away.`;
      } else {
        reply = "I couldn't make that change without breaking a constraint or the route, so I left your plan untouched.";
      }
    }
